...
void PrintHelp()
{
    ...
    std::cout << "\t>Write [index]- Writes an ULONG to pipe [index][payload]\n";
...
void DoWrite(size_t Index, ULONG Payload)
{
    DWORD noBytes = 0;

    gData[Index].input = Payload;
...
int main()
{
    ...
    while (true)
    {
        ...
        {
            std::cin >> index >> payload;
            DoWrite(index, payload);
        }

...
IrpManager::~IrpManager()
{
    ...
    auto irp = IoCsqRemoveNextIrp(&pendingWrites.Csq, nullptr);
    while (irp)
    {
...
)
{
    ...
    // No write pending. Mark read successful with 0 bytes
    if (!writeEnqueued)
    {
    ...
    // Remove the IRP from pending queue
...
)
{
    ...
    if (writeEnqueued)
    {
        // There is already a write enqueued => complete the current IRP
    ...
    // We have no other write pending so we pend current write
    writeEnqueued = true;
...
NTSTATUS IrpManager::CancelIrp(_In_ PIRP Irp)
{
    ...
    auto irp = IoCsqRemoveNextIrp(&pendingWrites.Csq, stackLocation->FileObject);
    while (irp)
    {
        irp->IoStatus.Status = STATUS_CANCELLED;
        irp->IoStatus.Information = 0;
